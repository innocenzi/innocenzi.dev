				...
				<article v-for="article in articles" :key="article.title" class="md:grid md:grid-cols-4 md:items-baseline">
					<div class="group relative flex flex-col items-start md:col-span-3">
						<h2 class="text-base font-semibold tracking-tight text-zinc-100">
							<div class="absolute -inset-x-4 -inset-y-6 z-0 scale-95 bg-zinc-800/50 opacity-0 transition group-hover:scale-100 group-hover:opacity-100 sm:-inset-x-6 sm:rounded-2xl" />
						...
						>
							Read article
							<Icon name="heroicons-solid:arrow-narrow-right" class="ml-2 size-4" />
						</div>
					</div>
					<div
						class="relative z-10 order-first mb-3 mt-1 hidden flex-col justify-center text-sm text-zinc-500 md:flex"
					>
						<time
							:datetime="article.created_at"
