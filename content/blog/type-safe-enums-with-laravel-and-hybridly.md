...
This transformer checks if the enum being transformed to a TypeScript type implements [`Hybridable`](https://github.com/hybridly/hybridly/blob/0.x/packages/laravel/src/Support/Hybridable.php). If it does, it recursively handles nested `Hybridable` properties returned by `toHybridArray()` and JSON-encodes the result, which would actually produce a valid TypeScript interface.
